...
%.js.check: %.js
	...
	echo -n "/*global alert, confirm, window, localStorage, $$, _, printf, escapeHtml, gfHtmlID, cloneObject, gfHasLocalStorage, gfStoreLocal, gfGetLocal, Poll" >> /tmp/$*.js
	echo -n ", GPGAuth" >> /tmp/$*.js
	echo -n " */" >> /tmp/$*.js
	cat $*.js >> /tmp/$*.js
	jslint /tmp/$*.js
