...
var PublicKey = function (asciikey) {
	...
	this.name_comment_mail = this.gp.find_id_packet().id;
	if (this.name_comment_mail.match(/.* \(.*\) <.*>/)) {
		this.name    = this.name_comment_mail.replace(/(.*) \(.*\) <.*>/,"$1");
		this.comment = this.name_comment_mail.replace(/.* \((.*)\) <.*>/,"$1");
	} else {
		this.name = this.name_comment_mail.replace(/(.*) <.*>/,"$1");
		this.comment = "";
	}
	this.mail = this.name_comment_mail.replace(/.*<(.*)>/,"$1");
	// FIXME: parse id and fingerprint
	this.fingerprint = "DEAD BEEF 0000 CAFE BABE  C00L D00D 0000 BAAD FEED";
	this.fingerprint = "D19E 04A8 8895 020A 8DF6  0092 3501 1A32 491A 3D9C";
	this.ascii = asciikey;
};
